...
\begin{luacode*}
	function rubricRow(title, weight)
		return {
			title = title,
			weight = weight,
			criteria = { "", "", "", "", "", "" },
			
			setCriterion = function(self, index, text)
				self.criteria[index+1] = text
				return self
			end,
			
			addParagraph = function(self, startIndex, fmt, args)
				for i,par in ipairs(args) do
					critIndex = startIndex + i
					print("****", i, par, critIndex)
					crit = self.criteria[critIndex] or ""
					if crit ~= "" then
						crit = crit .. "\\par "
					end
					crit = crit .. string.format(fmt, par)
					self.criteria[critIndex] = crit
				end
				return self
			end,
		}
	...
	function printRubric(rubric)
		...
		for rowIndex, row in ipairs(rubric) do
			...
			tex.print(row.title, "}{", string.format("%d", row.weight), "\\%}")
			
			for critIndex, criterion in ipairs(row.criteria) do
				tex.print("\\grade")
				if critIndex == 1 then
					tex.print("\\fail")
				end
...
\begin{luacode*}
	rubric = {
		rubricRow("Breadth of reading", 5)
			: addParagraph(0, "Fewer than the four suggested articles are referenced.", {0})
			: addParagraph(1, "All four of the suggested articles are referenced.", {0,0,0,0,0})
			: addParagraph(2, "A further %d sources are also referenced.", {6, 12, 18, 24})
		,
		rubricRow("Depth of insight", 15)
			: addParagraph(0, "%s insight is demonstrated.",
...
%        \grade\fail	Fewer than the four suggested articles are referenced.
%        \grade		All four of the suggested articles are referenced.
%        \grade		All four of the suggested articles are referenced.
