...
In this assignment, you will port a 2D game for Windows PC. Your game will be written in C++ using the Simple DirectMedia Layer (SDL) library. You will work in small groups (typically, 3--4 students).

The quality of a digital game is not only determined by the fun associated with its design, but also on the soundness of its architecture. Put simply, games with serious bugs do not sell well. As such, the value of software engineering principles and practices cannot be emphasised enough! This assignment enable of a game development project that reflects industry practice.
...
\begin{enumerate}[label=(\Alph*)]
    \item \textbf{Prepare}, as a \textbf{group}, a plan which will:
    	\begin{enumerate}[label=\roman*.]
    	    	\item \textbf{identify} the skills and time available to complete porting the game;
    	    	\item \textbf{iterate} upon \textbf{and improve} the game port;
    		\item \textbf{describe} the key user stories that comprise the game's design;
    		\item and \textbf{highlight} the stories that will comprise a minimal viable product.
	\end{enumerate}
    \item \textbf{Implement}, as a \textbf{group}, an initial production prototype of the game in SDL which will:
    	\begin{enumerate}[label=\roman*.]
    		\item \textbf{illustrate} the core game mechanic;
    		\item \textbf{apply} at least \textbf{one} interesting algorithm.
	\end{enumerate}
    ...
    \item \textbf{Write}, as a \textbf{group}, a presentation slide-deck and 2-page handout that will:
    	\begin{enumerate}[label=\roman*.]
    		\item \textbf{outline} the technology and engineering behind the game;
    		\item and \textbf{explain} the game design's commercial feasibility.
	\end{enumerate}
    \item \textbf{Present}, as a \textbf{group}, a 15-minute `technical demo` that will:
    	\begin{enumerate}[label=\roman*.]
    		\item \textbf{clarify} the technical and engineering content of your presentation slide-deck and handout;
